...
import {
  ...
  Grid, Alert, CircularProgress, IconButton, ToggleButton,
                ...
                <Paper elevation={0} sx={{
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2.5 }}>
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" color="text.secondary">First photo = cover · max 6</Typography>
                    </Box>
                  </Box>

                  <Box sx={{ border: '2px dashed', borderColor: 'rgba(27,58,92,0.2)', borderRadius: 3,
                    p: 3, textAlign: 'center', mb: 2, background: 'rgba(27,58,92,0.02)',
                    '&:hover': { borderColor: 'primary.main' } }}>
                    <Icon icon="mdi:cloud-upload-outline" width={40} color="#1B3A5C" />
                    <Typography variant="body2" color="text.secondary" mt={1} mb={2}>
                      Select new photos to add
                    </Typography>
                    <Button variant="outlined" component="label" size="small"
                      startIcon={<Icon icon="mdi:image-plus-outline" />} sx={{ borderRadius: 2, mr: 1 }}>
                      Select Photos
                      <input type="file" hidden accept="image/*" multiple onChange={(e) => setFiles(e.target.files)} />
                    </Button>
                    {files.length > 0 && (
                  ...
                  <Button fullWidth variant="contained" color="primary" onClick={handleImageSubmit}
                    ...
                    sx={{ borderRadius: 2, mb: 2, background: 'linear-gradient(90deg, #1B3A5C, #2E5F8A)' }}>
                  ...
                  {formData.imageUrls.length > 0 ? (
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1.5 }}>
                      {formData.imageUrls.map((url, idx) => (
                        <Box key={url} sx={{ position: 'relative', borderRadius: 2, overflow: 'hidden', aspectRatio: '4/3' }}>
                    ...
                    <Box sx={{ textAlign: 'center', py: 2, opacity: 0.4 }}>
                      ...
                      <Typography variant="caption" display="block" color="text.secondary" mt={0.5}>No photos yet</Typography>
                    </Box>
                  )}
                </Paper>
              </motion.div>
          ...
          <motion.div {...fadeUp(0.25)}>
            ...
            <Button fullWidth variant="contained" color="primary" size="large" type="submit"
              ...
              sx={{
                ...
                background: 'linear-gradient(90deg, #0F2240 0%, #1B3A5C 100%)',
                boxShadow: '0 4px 20px rgba(27,58,92,0.35)',
                '&:hover': { background: 'linear-gradient(90deg, #000d1a 0%, #0F2240 100%)' },
              }}
              startIcon={loading ? <CircularProgress size={22} color="inherit" /> : <Icon icon="mdi:content-save-outline" width={22} />}>
